...
exports.AddOrder = async (req, res) => {
  try {
    ...
    const cart = await Cart.findOne({
      userId: req.user.id,
    }).populate('products.productId').populate('couponCode');
...
exports.cashOnDelivery = async (req, res, next) => {
  ...
  try {
    ...
    if (!req.body.status) {
      ...
      order.paymentMethod = req.body.status;
    }
...
exports.readyOrder1 = async (req, res) => {
  try {
    const orderId = req.params.orderId;
    const vendorId = req.user.id;
    ...
    // Check if the order is assigned to the vendor making the request
    if (order.products.createdBy.toString() !== vendorId.toString()) {
      return res.status(403).json({ error: 'You are not authorized to update this order status' });
    }
